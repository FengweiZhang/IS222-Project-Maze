...
//	ILI9341_DispString_EN(10, 30 ,"WELCOME!");
	ILI9341_DispString_EN(12, 62 ,"MAZE GAME");

//	LCD_SetColors(BLACK, WHITE);
//	ILI9341_DrawRectangle(39, 199, 162, 52, 1);
//	LCD_SetColors(WHITE, WHITE);
//	ILI9341_DrawRectangle(40, 200, 160, 50, 1);

	LCD_SetColors(RED, WHITE);
	ILI9341_DispString_EN(45, 210 ,"START!");
...
void LCD_MAZE_DrawMap()
{
	...
	for (int i=0; i<MAZE_X; i+=MAZE_X-1)
	{
		for (int j=0; j<MAZE_Y; j++)
		{
			ILI9341_DrawRectangle(MAZE_START_X+i*CELL_SIZE, MAZE_START_Y+j*CELL_SIZE, CELL_SIZE, CELL_SIZE, 1);
		}
	}
	for (int j=0; j<MAZE_Y; j+=MAZE_Y-1)
	{
	...
	for (int i=maze.player[0]-5; i<=maze.player[0]+5; i++)
	{
		for(int j=maze.player[1]-5; j<=maze.player[1]+5; j++)
		{
			if (i < 0 || i >= MAZE_X ||j <0 || j>=MAZE_Y) continue;
			if (maze.map[i][j] == WALL_S)
			{
				ILI9341_DrawRectangle(MAZE_START_X+i*CELL_SIZE, MAZE_START_Y+j*CELL_SIZE, CELL_SIZE, CELL_SIZE, 1);
			}
		}
	...
	ILI9341_DrawRectangle(MAZE_START_X+maze.enter[0]*CELL_SIZE, MAZE_START_Y+maze.enter[1]*CELL_SIZE, CELL_SIZE, CELL_SIZE, 1);
	LCD_SetTextColor(GREEN);
	ILI9341_DrawRectangle(MAZE_START_X+maze.exit[0]*CELL_SIZE, MAZE_START_Y+maze.exit[1]*CELL_SIZE, CELL_SIZE, CELL_SIZE, 1);

	LCD_SetTextColor(BLACK);
	for (int i=maze.enter[0]-4; i<=maze.enter[0]+4; i++)
	{
		for(int j=maze.enter[1]-4; j<=maze.enter[1]+4; j++)
		{
			if (i < 0 || i >= MAZE_X ||j <0 || j>=MAZE_Y) continue;
			if (maze.map[i][j] == WALL_S)
			{
	...
	for (int i=maze.exit[0]-4; i<=maze.exit[0]+4; i++)
	{
		for(int j=maze.exit[1]-4; j<=maze.exit[1]+4; j++)
		{
			if (i < 0 || i >= MAZE_X ||j <0 || j>=MAZE_Y) continue;
			if (maze.map[i][j] == WALL_S)
			{
...
{
	int radius = (CELL_SIZE-3)/2;
	LCD_SetTextColor(BLUE);
	ILI9341_DrawCircle(MAZE_START_X+maze.player[0]*CELL_SIZE+(CELL_SIZE-1)/2,
						MAZE_START_Y+maze.player[1]*CELL_SIZE+(CELL_SIZE-1)/2,
						radius-light, 1);
}
...
{
	LCD_SetColors(BLACK, WHITE);
	ILI9341_DrawRectangle(UP_BUTTON_X, UP_BUTTON_Y, BUTTON_WIDTH, BUTTON_HEIGHT, 0);
	ILI9341_DrawRectangle(DOWN_BUTTON_X, DOWN_BUTTON_Y, BUTTON_WIDTH, BUTTON_HEIGHT, 0);
	ILI9341_DrawRectangle(LEFT_BUTTON_X, LEFT_BUTTON_Y, BUTTON_WIDTH, BUTTON_HEIGHT, 0);
	ILI9341_DrawRectangle(RIGHT_BUTTON_X, RIGHT_BUTTON_Y, BUTTON_WIDTH, BUTTON_HEIGHT, 0);
	LCD_SetFont(&Font8x16);
	ILI9341_DispString_EN(UP_BUTTON_X+22, UP_BUTTON_Y+7, "UP");
	ILI9341_DispString_EN(DOWN_BUTTON_X+14, DOWN_BUTTON_Y+7, "DOWN");
	ILI9341_DispString_EN(LEFT_BUTTON_X+14, LEFT_BUTTON_Y+7, "LEFT");
	ILI9341_DispString_EN(RIGHT_BUTTON_X+10, RIGHT_BUTTON_Y+7, "RIGHT");
}
...
void LCD_MAZE_DrawStep()
{
	LCD_SetColors(BLACK, WHITE);
	LCD_SetFont(&Font8x16);
	ILI9341_DispString_EN(STEP_X, STEP_Y, "STEPS:");
	char cStr [ 100 ] = {0};
	sprintf(cStr,"%lu",maze.step);
	LCD_SetColors(RED, WHITE);
	LCD_SetFont(&Font16x24);
	ILI9341_DispString_EN(STEP_X, STEP_Y+16, cStr);
...
void LCD_MAZE_DrawBest()
{
	LCD_SetColors(BLACK, WHITE);
	LCD_SetFont(&Font8x16);
	ILI9341_DispString_EN(BEST_X, BEST_Y, "BEST:");
	char cStr [ 100 ] = {0};
	sprintf(cStr,"%lu",maze.best);
//	LCD_SetColors(RED, WHITE);
	LCD_SetFont(&Font16x24);
	ILI9341_DispString_EN(BEST_X, BEST_Y+16, cStr);
}

void LCD_MAZE_DrawRestart()
{
	LCD_SetColors(BLACK, WHITE);
	ILI9341_DrawRectangle(RESTART_BUTTON_X, RESTART_BUTTON_Y, BUTTON_WIDTH, BUTTON_HEIGHT, 0);
	LCD_SetFont(&Font8x16);
	ILI9341_DispString_EN(RESTART_BUTTON_X+2, RESTART_BUTTON_Y+7, "RESTART");
...
void LCD_MAZE_BUTTON_Down(uint8_t type)
{
	...
	switch (type)
	{
		case UP_BUTTON:
		{
			ILI9341_DrawRectangle(UP_BUTTON_X, UP_BUTTON_Y, BUTTON_WIDTH, BUTTON_HEIGHT, 1);
		...
		case DOWN_BUTTON:
		{
			ILI9341_DrawRectangle(DOWN_BUTTON_X, DOWN_BUTTON_Y, BUTTON_WIDTH, BUTTON_HEIGHT, 1);
			LCD_SetTextColor(WHITE);
			ILI9341_DispString_EN(DOWN_BUTTON_X+14, DOWN_BUTTON_Y+7, "DOWN");
			break;
		}
		case LEFT_BUTTON:
		{
			ILI9341_DrawRectangle(LEFT_BUTTON_X, LEFT_BUTTON_Y, BUTTON_WIDTH, BUTTON_HEIGHT, 1);
			LCD_SetTextColor(WHITE);
			ILI9341_DispString_EN(LEFT_BUTTON_X+14, LEFT_BUTTON_Y+7, "LEFT");
			break;
		}
		case RIGHT_BUTTON:
		{
			ILI9341_DrawRectangle(RIGHT_BUTTON_X, RIGHT_BUTTON_Y, BUTTON_WIDTH, BUTTON_HEIGHT, 1);
			LCD_SetTextColor(WHITE);
			ILI9341_DispString_EN(RIGHT_BUTTON_X+10, RIGHT_BUTTON_Y+7, "RIGHT");
			break;
		}
		case RESTART_BUTTON:
		{
			ILI9341_DrawRectangle(RESTART_BUTTON_X, RESTART_BUTTON_Y, BUTTON_WIDTH, BUTTON_HEIGHT, 1);
			LCD_SetTextColor(WHITE);
			ILI9341_DispString_EN(RESTART_BUTTON_X+2, RESTART_BUTTON_Y+7, "RESTART");
			break;
		}
		...
		{
//			LCD_SetColors(WHITE, WHITE);
//			LCD_Clear(UP_BUTTON_X, UP_BUTTON_Y, BUTTON_WIDTH, BUTTON_HEIGHT);
//			LCD_Clear(DOWN_BUTTON_X, DOWN_BUTTON_Y, BUTTON_WIDTH, BUTTON_HEIGHT);
//			LCD_Clear(LEFT_BUTTON_X, LEFT_BUTTON_Y, BUTTON_WIDTH, BUTTON_HEIGHT);
//			LCD_Clear(RIGHT_BUTTON_X, RIGHT_BUTTON_Y, BUTTON_WIDTH, BUTTON_HEIGHT);
//			LCD_SetColors(BLACK, WHITE);
//			ILI9341_DrawRectangle(UP_BUTTON_X, UP_BUTTON_Y, BUTTON_WIDTH, BUTTON_HEIGHT, 0);
//			ILI9341_DrawRectangle(DOWN_BUTTON_X, DOWN_BUTTON_Y, BUTTON_WIDTH, BUTTON_HEIGHT, 0);
//			ILI9341_DrawRectangle(LEFT_BUTTON_X, LEFT_BUTTON_Y, BUTTON_WIDTH, BUTTON_HEIGHT, 0);
...
void LCD_WIM_DrawWin()
{
	LCD_SetColors(RED,WHITE);
	LCD_SetFont(&Font24x32);
	ILI9341_DispString_EN(WIN_X, WIN_Y, "YOU WIN!");
}
